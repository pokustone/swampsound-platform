...
const fmtT = ts => { if (!ts) return "—"; const d = Date.now() - ts; if (d < 60000) return S.now; if (d < 3600000) return `${Math.floor(d / 60000)}m`; if (d < 86400000) return `${Math.floor(d / 3600000)}h`; return new Date(ts).toLocaleDateString("cs-CZ"); };
...
function EmbedInput({ onAdd, placeholder }) {
  ...
  return <div style={{ marginBottom: 12 }}><div style={{ display: "flex", gap: 8 }}><Inp label="" value={url} onChange={v => { setUrl(v); setErr(""); }} placeholder={placeholder} error={err} style={{ flex: 1, marginBottom: 0 }} /><Btn v="small" onClick={() => { if (!p) { setErr(S.embedBad); return; } onAdd({ url, ...p }); setUrl(""); }}>{S.add}</Btn></div>{p && <div style={{ fontSize: 11, color: C.grnL, marginTop: 6 }}>{p.type === "youtube" ? `YouTube (${p.id})` : "SoundCloud"}</div>}</div>;
}
...
function Register({ onNav }) {
  ...
  return <div style={{ maxWidth: 400, margin: "0 auto", padding: "60px 20px" }}><div style={{ textAlign: "center", marginBottom: 30 }}><h1 style={{ fontSize: 22, color: C.acc, letterSpacing: 2, margin: 0, textTransform: "uppercase", fontFamily: "inherit" }}>{S.regTitle}</h1></div><div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 24 }}><div style={{ width: 26, height: 26, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: "bold", background: C.acc, color: "#0a0f0a" }}>1</div><div style={{ width: 36, height: 1, background: step >= 2 ? C.acc : C.brd }} /><div style={{ width: 26, height: 26, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: "bold", background: step >= 2 ? C.acc : C.brd, color: step >= 2 ? "#0a0f0a" : C.txM }}>2</div></div>{step === 1 && <Crd><Inp label={S.regInviteCode} value={tok} onChange={v => { setTok(v.toUpperCase()); setTokErr(""); }} placeholder={S.regInvitePh} error={tokErr} required /><Btn onClick={check} style={{ width: "100%" }}>{S.regVerify}</Btn></Crd>}{step === 2 && <Crd><Inp label={S.regFullName} value={f.fn} onChange={v => up("fn", v)} error={errs.fn} required /><Inp label={S.loginEmail} type="email" value={f.em} onChange={v => up("em", v)} error={errs.em} required /><Inp label={S.regPhone} value={f.ph} onChange={v => up("ph", v)} placeholder={S.regPhonePh} error={errs.ph} required /><Inp label={S.loginPassword} type="password" value={f.pw} onChange={v => up("pw", v)} placeholder={S.regPwPh} error={errs.pw} required /><Inp label={S.regPwAgain} type="password" value={f.pw2} onChange={v => up("pw2", v)} error={errs.pw2} required /><Inp label={S.regNickname} value={f.nick} onChange={v => up("nick", v)} /><Btn onClick={submit} disabled={sub} style={{ width: "100%" }}>{sub ? S.regSubmitting : S.regSubmit}</Btn></Crd>}<div style={{ textAlign: "center", marginTop: 10 }}><button onClick={() => onNav("landing")} style={{ background: "none", border: "none", color: C.txM, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>{S.back}</button></div></div>;
}
...
function EvDetail({ eid, user, events, onNav }) {
  ...
  return <div style={{ maxWidth: 620, margin: "0 auto", padding: "28px 20px" }}>
    ...
    {ev.flyer && <Sec title={S.evFlyer}><Crd><img src={ev.flyer} style={{ maxWidth: "100%", borderRadius: 4, display: "block" }} /></Crd></Sec>}
    ...
    <Sec title="Fotogalerie" right={isA && <Btn v="small" onClick={() => setShowUp(showUp === "photos" ? null : "photos")}>{showUp === "photos" ? S.close : S.galAddPhotos}</Btn>}>
      {showUp === "photos" && <Crd style={{ borderColor: C.acc }}><DropZone onFiles={handlePhotoFiles} label={S.galDropDetail} accept="image/*" />{upPhotos.length > 0 && <><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>{upPhotos.map(p => <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 6 }}><PhotoImg photo={p} size={48} /><span style={{ fontSize: 11, color: C.tx }}>{p.caption}</span><button onClick={() => setUpPhotos(ps => ps.filter(x => x.id !== p.id))} style={{ background: "none", border: "none", color: C.red, cursor: "pointer", fontSize: 14, padding: 0 }}>×</button></div>)}</div><Btn onClick={savePhotos} disabled={saving}>{saving ? S.saving : S.galSaveCount(upPhotos.length)}</Btn></>}</Crd>}
      {gals.map(g => <Crd key={g.id}><div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>{(g.photos || []).map((p, i) => <div key={p.id} style={{ position: "relative", flexShrink: 0 }}><PhotoImg photo={p} size={90} onClick={() => { setLbPhotos(g.photos); setLbIdx(i); }} />{isA && <button onClick={() => { if (confirm(S.galDeletePhoto)) DB.removePhotoFromGallery(g.id, p.id).catch(e => alert(e.message)); }} style={{ position: "absolute", top: 2, right: 2, background: "rgba(0,0,0,.7)", border: "none", color: C.red, cursor: "pointer", fontSize: 12, width: 20, height: 20, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>×</button>}</div>)}</div>{isA && <button onClick={() => { if (confirm(S.galDeleteConfirm(g.title || ""))) DB.deleteGalleryDoc(g.id).catch(e => alert(e.message)); }} style={{ background: "none", border: "none", color: C.red, fontSize: 10, cursor: "pointer", fontFamily: "inherit", marginTop: 6, opacity: .7 }}>Smazat galerii</button>}</Crd>)}
      {!gals.length && !showUp && <Crd><div style={{ fontSize: 12, color: C.txM, textAlign: "center" }}>Žádné fotky.</div></Crd>}
    </Sec>
    <Sec title="Audio" right={isA && <Btn v="small" onClick={() => setShowUp(showUp === "audio" ? null : "audio")}>{showUp === "audio" ? S.close : "+ Audio"}</Btn>}>
      {showUp === "audio" && <Crd style={{ borderColor: C.acc }}><DropZone onFiles={handleAudioFiles} label={S.mediaAudioDropDetail} accept="audio/*" /><div style={{ fontSize: 10, letterSpacing: 2, color: C.acc, textTransform: "uppercase", margin: "8px 0 10px" }}>Nebo SoundCloud</div><EmbedInput onAdd={({ url }) => addScAudio(url)} placeholder="https://soundcloud.com/..." /></Crd>}
      {auds.map(a => <Crd key={a.id}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div style={{ flex: 1 }}><AudioPlayer track={a} /></div>{isA && <button onClick={() => { if (confirm(S.mediaDeleteAudio)) DB.deleteAudioDoc(a.id).catch(e => alert(e.message)); }} style={{ background: "none", border: `1px solid ${C.brd}`, color: C.red, padding: "4px 10px", borderRadius: 4, fontSize: 10, cursor: "pointer", fontFamily: "inherit", flexShrink: 0, marginLeft: 8 }}>✕</button>}</div></Crd>)}
      {!auds.length && !showUp && <Crd><div style={{ fontSize: 12, color: C.txM, textAlign: "center" }}>Žádné nahrávky.</div></Crd>}
    </Sec>
    <Sec title="Videa" right={isA && <Btn v="small" onClick={() => setShowUp(showUp === "video" ? null : "video")}>{showUp === "video" ? S.close : "+ Video"}</Btn>}>
      {showUp === "video" && <Crd style={{ borderColor: C.acc }}><EmbedInput onAdd={addVideo} placeholder="https://youtube.com/watch?v=..." /></Crd>}
      {vids.map(v => { const p = parseUrl(v.embedUrl); return <Crd key={v.id}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div style={{ flex: 1 }}>{p?.type === "youtube" ? <YtEmbed videoId={p.id} title={v.title} /> : <div>{v.title}</div>}</div>{isA && <button onClick={() => { if (confirm(S.mediaDeleteVideo)) DB.deleteVideoDoc(v.id).catch(e => alert(e.message)); }} style={{ background: "none", border: `1px solid ${C.brd}`, color: C.red, padding: "4px 10px", borderRadius: 4, fontSize: 10, cursor: "pointer", fontFamily: "inherit", flexShrink: 0, marginLeft: 8 }}>✕</button>}</div></Crd>; })}
      {!vids.length && !showUp && <Crd><div style={{ fontSize: 12, color: C.txM, textAlign: "center" }}>Žádná videa.</div></Crd>}
    </Sec>
    <Sec title={S.evDiscussion(msgCount)} right={<Btn v="small" onClick={() => setShowChat(!showChat)}>{showChat ? S.hide : S.show}</Btn>}>
      {showChat && <div style={{ height: 350 }}><MiniChat user={user} room={eid} events={events} /></div>}
      {!showChat && <Crd hov onClick={() => setShowChat(true)}><div style={{ fontSize: 12, color: C.txM }}>{msgCount > 0 ? S.chatMsgCount(msgCount) : S.chatStart}</div></Crd>}
    </Sec>
    {isA && <Sec title={S.adminDocs} right={<Btn v="small" onClick={() => setShowAdDocs(!showAdDocs)}>{showAdDocs ? S.hide : S.show}</Btn>}>
      {showAdDocs && <Crd style={{ borderColor: C.accD }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <input value={adLink.title} onChange={e => setAdLink(p => ({ ...p, title: e.target.value }))} placeholder={S.adminDocsTitle} style={{ flex: 1, padding: "6px 10px", background: C.grnD, border: `1px solid ${C.brd}`, borderRadius: 4, color: C.tx, fontSize: 11, fontFamily: "inherit", outline: "none" }} />
  ...
  return <div style={{ marginBottom: 14 }}>
    ...
    {lineup.map((a, i) => <div key={i} style={{ background: C.grnD, border: `1px solid ${C.brd}`, borderRadius: 4, padding: 10, marginBottom: 8 }}>
      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: (a.links?.length > 0) ? 8 : 0 }}>
        <input value={a.name} onChange={e => setName(i, e.target.value)} placeholder={S.lineupName} style={{ flex: 1, padding: "7px 10px", background: C.srf, border: `1px solid ${C.brd}`, borderRadius: 4, color: C.tx, fontSize: 12, fontFamily: "inherit", outline: "none" }} />
        <button onClick={() => addLink(i)} title={S.lineupAddLink} style={{ background: "none", border: `1px solid ${C.brd}`, color: C.acc, width: 28, height: 28, borderRadius: 4, cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center" }}>+🔗</button>
...
function Events({ user, events, onNav }) {
  ...
  const del = async (ev) => {
    if (!confirm(S.evDeleteConfirm(ev.title))) return;
...
function GalList({ onNav, user, events }) {
  ...
  const save = async () => {
    ...
    const title = eventLink ? (events.find(e => e.id === eventLink)?.title || S.galTitle) + " — Fotky" : folderName.trim();
    if (!title) { alert(S.galNeedName); return; }
  ...
  return <div style={{ maxWidth: 620, margin: "0 auto", padding: "28px 20px" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}><h1 style={{ fontSize: 20, color: C.acc, margin: 0, fontFamily: "inherit", letterSpacing: 2, textTransform: "uppercase" }}>Galerie</h1>{isA && <Btn v="secondary" onClick={() => showAdd ? cancelAdd() : setShowAdd(true)}>{showAdd ? S.cancel : S.galNewFolder}</Btn>}</div>
    {showAdd && <Crd style={{ borderColor: C.acc }}>
      <div style={{ marginBottom: 14 }}><label style={{ display: "block", fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: C.acc, marginBottom: 5, fontFamily: "inherit" }}>K akci</label><select value={eventLink} onChange={e => { setEventLink(e.target.value); if (e.target.value) setFolderName(""); }} style={{ width: "100%", padding: "10px 12px", background: C.grnD, border: `1px solid ${C.brd}`, borderRadius: 4, color: C.tx, fontSize: 13, fontFamily: "inherit" }}><option value="">{S.galOwnName}</option>{events.map(ev => <option key={ev.id} value={ev.id}>{ev.title}</option>)}</select></div>
      {!eventLink && <Inp label={S.galFolderName} value={folderName} onChange={setFolderName} required />}
      ...
      <DropZone onFiles={handleFiles} label={S.galDrop} accept="image/*" />{upPhotos.length > 0 && <><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>{upPhotos.map(p => <PhotoImg key={p.id} photo={p} size={48} />)}</div><Btn onClick={save} disabled={saving}>{saving ? S.saving : S.galSaveCount(upPhotos.length)}</Btn></>}
    </Crd>}
    {gals.map(g => { const ev = g.eventId ? events.find(e => e.id === g.eventId) : null; return <Crd key={g.id} hov onClick={() => addToId === g.id ? null : onNav("gallery:" + g.id)}>
      ...
      {addToId === g.id && <div style={{ marginTop: 12, borderTop: `1px solid ${C.brd}`, paddingTop: 12 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 8 }}>{upPhotos.length > 0 && <Btn v="small" onClick={saveAddTo} disabled={saving}>{saving ? S.saving : S.galAddCount(upPhotos.length)}</Btn>}<button onClick={e => { e.stopPropagation(); setAddToId(null); setUpPhotos([]); }} style={{ background: "none", border: "none", color: C.txM, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>Zrušit</button></div>
      </div>}
    </Crd>; })}
  </div>;
}
function GalDetail({ gid, onNav, backTo, user }) {
  ...
  return <div style={{ maxWidth: 620, margin: "0 auto", padding: "28px 20px" }}>
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}><h1 style={{ fontSize: 20, color: C.acc, margin: 0, fontFamily: "inherit" }}>{g.title}</h1>{isA && <Btn v="small" onClick={() => setShowUp(!showUp)}>{showUp ? S.cancel : S.galAddPhotos}</Btn>}</div>
    {showUp && <Crd style={{ borderColor: C.acc, marginBottom: 16 }}><DropZone onFiles={handleFiles} label={S.galAddMore} accept="image/*" />{upPhotos.length > 0 && <><div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>{upPhotos.map(p => <PhotoImg key={p.id} photo={p} size={48} />)}</div><Btn onClick={savePhotos} disabled={saving}>{saving ? S.saving : S.galAddCount(upPhotos.length)}</Btn></>}</Crd>}
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: 10 }}>{(g.photos || []).map((p, i) => <div key={p.id} style={{ position: "relative" }}><PhotoImg photo={p} size={160} onClick={() => setLbIdx(i)} />{isA && <button onClick={e => { e.stopPropagation(); if (confirm(S.galDeletePhoto)) DB.removePhotoFromGallery(g.id, p.id).catch(err => alert(err.message)); }} style={{ position: "absolute", top: 4, right: 4, background: "rgba(0,0,0,.7)", border: "none", color: C.red, cursor: "pointer", fontSize: 12, width: 22, height: 22, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>×</button>}<div style={{ fontSize: 11, color: C.txM, marginTop: 4, textAlign: "center" }}>{p.caption}</div></div>)}</div>
  </div>;
}
...
function MediaPage({ user, events, onNav }) {
  ...
  return <div style={{ maxWidth: 620, margin: "0 auto", padding: "28px 20px" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}><h1 style={{ fontSize: 20, color: C.acc, margin: 0, fontFamily: "inherit", letterSpacing: 2, textTransform: "uppercase" }}>Audio / Video</h1>{isA && <Btn v="secondary" onClick={() => setShowAdd(!showAdd)}>{showAdd ? S.cancel : S.add}</Btn>}</div>
    ...
    {showAdd && <Crd style={{ borderColor: C.acc }}><Inp label={S.evName} value={addTitle} onChange={setAddTitle} placeholder={tab === "audio" ? S.mediaAudioName : S.mediaVideoName} /><div style={{ marginBottom: 14 }}><label style={{ display: "block", fontSize: 10, letterSpacing: 2, textTransform: "uppercase", color: C.acc, marginBottom: 5, fontFamily: "inherit" }}>K akci (volitelné)</label><select value={addEvent} onChange={e => setAddEvent(e.target.value)} style={{ width: "100%", padding: "10px 12px", background: C.grnD, border: `1px solid ${C.brd}`, borderRadius: 4, color: C.tx, fontSize: 13, fontFamily: "inherit" }}><option value="">{S.mediaToEventNone}</option>{events.map(ev => <option key={ev.id} value={ev.id}>{ev.title}</option>)}</select></div>
    {tab === "audio" && <><DropZone onFiles={handleAudioFiles} label={S.mediaAudioDrop} accept="audio/*" /><div style={{ fontSize: 10, letterSpacing: 2, color: C.acc, textTransform: "uppercase", margin: "8px 0 10px" }}>Nebo SoundCloud</div><EmbedInput onAdd={({ url }) => addScTrack(url)} placeholder="https://soundcloud.com/..." /></>}
    {tab === "video" && <EmbedInput onAdd={addVid} placeholder="https://youtube.com/watch?v=..." />}
    </Crd>}
    {tab === "audio" && auds.map(a => { const ev = a.eventId ? events.find(e => e.id === a.eventId) : null; return <Crd key={a.id}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div style={{ flex: 1 }}><AudioPlayer track={a} />{ev && <div style={{ marginTop: 6 }}><EvLink event={ev} onNav={onNav} /></div>}</div>{isA && <button onClick={() => { if (confirm(S.mediaDeleteAudio)) DB.deleteAudioDoc(a.id).catch(e => alert(e.message)); }} style={{ background: "none", border: `1px solid ${C.brd}`, color: C.red, padding: "4px 10px", borderRadius: 4, fontSize: 10, cursor: "pointer", fontFamily: "inherit", flexShrink: 0, marginLeft: 8 }}>✕</button>}</div></Crd>; })}
    {tab === "video" && vids.map(v => { const p = parseUrl(v.embedUrl); const ev = v.eventId ? events.find(e => e.id === v.eventId) : null; return <Crd key={v.id}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}><div style={{ flex: 1 }}>{p?.type === "youtube" ? <YtEmbed videoId={p.id} title={v.title} /> : <div>{v.title}</div>}{ev && <div style={{ marginTop: 6 }}><EvLink event={ev} onNav={onNav} /></div>}</div>{isA && <button onClick={() => { if (confirm(S.mediaDeleteVideo)) DB.deleteVideoDoc(v.id).catch(e => alert(e.message)); }} style={{ background: "none", border: `1px solid ${C.brd}`, color: C.red, padding: "4px 10px", borderRadius: 4, fontSize: 10, cursor: "pointer", fontFamily: "inherit", flexShrink: 0, marginLeft: 8 }}>✕</button>}</div></Crd>; })}
  </div>;
}
...
function Admin({ user }) {
  ...
  const sendEmail = async () => {
    ...
    if (users.find(u => u.email === mEm)) { setMSent({ err: S.adminRegistered }); return; }
  ...
  const tabs = [{ id: "invites", l: S.adminTabQr }, { id: "email", l: S.notifEmail }, { id: "users", l: S.adminTabUsers }];
  ...
  return <div style={{ maxWidth: 680, margin: "0 auto", padding: "28px 20px" }}>
    ...
    {tab === "invites" && <>
      <Crd style={{ borderColor: C.accD }}><div style={{ fontSize: 10, letterSpacing: 2, color: C.acc, textTransform: "uppercase", marginBottom: 14 }}>Generovat dávku</div><Inp label={S.adminBatchName} value={batchName} onChange={setBatchName} placeholder={S.adminBatchPh} required /><div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}><div style={{ flex: 1, minWidth: 80 }}><Inp label={S.adminBatchCount} type="number" value={bsz} onChange={setBsz} /></div><div style={{ flex: 1, minWidth: 80 }}><Inp label={S.adminBatchPerA4} type="number" value={perA4} onChange={setPerA4} /></div><Btn onClick={genBatch} disabled={!batchName.trim() || genLoading} style={{ marginBottom: 14 }}>{genLoading ? S.adminGenerating : S.adminGenerate}</Btn></div></Crd>
